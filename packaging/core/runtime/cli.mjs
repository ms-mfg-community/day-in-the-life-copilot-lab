...
async function main() {
  ...
  const workspace = realpathSync(findWorkspace(values.workspace));
  ...
  if (command === 'shell') return execute('/bin/bash', ['--noprofile', '--norc', '-i'], workspace, env);
  if (command === 'copilot') return execute(join(runtime, 'tools/node_modules/.bin/copilot'), copilotArguments(context, runtime, env, args), workspace, env);
  if (!args.length) throw new Error('lab-core exec requires a command after --');
  execute(args[0], args.slice(1), workspace, env);
}
